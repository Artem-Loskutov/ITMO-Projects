...
template<int nrows, int ncols> mat<nrows, ncols> identify() {
    mat<nrows, ncols> ret;
    for (int i = 0; i < nrows; ++i)
        for (int j = 0; j < ncols; ++j)
            ret[i][j] = 0.0;
    
    int m = nrows < ncols ? nrows : ncols;
    for (int i = 0; i < m; ++i) ret[i][i] = 1.0;

    return ret;
}

template<int n, int m>
vec<m> proj(const vec<n>& v) {
    vec<m> ret;
...
vec<m> embed(const vec<n>& v, double fill = 1.0) {
    vec<m> ret;
